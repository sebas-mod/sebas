    ...
    code: async (ctx) => {
        try {
            ...
            topUsers.forEach((user, index) => {
                ...
                resultText += quote(`${index + 1}. ${displayName} - Ganó: ${user.winGame}, Nivel: ${user.level}\n`);
            });

            if (userRank > 10) {
                const userStats = leaderboardData[userRank - 1];
                const displayName = `@${senderId}`;
                resultText += quote(`${userRank}. ${displayName} - Ganó: ${userStats.winGame}, Nivel: ${userStats.level}\n`);
            }
