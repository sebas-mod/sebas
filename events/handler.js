    ...
    try {
        ...
        for (const jid of m.participants) {
            ...
            const text = customText ?
                ...
                (type === "UserJoin" ?
                    quote(`#- Hola ${userTag} gracias por tu ingreso a ${metadata.subject}!`) :
                    quote(`#- ${userTag} byebye, se salió de ${metadata.subject}.`));
        ...
        if (config.system.reportErrorToOwner) await bot.core.sendMessage(`${config.owner.id}@s.whatsapp.net`, {
            text: `${quote("⚠️ Hay un error:")}\n` +
                `${quote("─────")}\n` +
                monospace(errorText)
        });
        await bot.core.sendMessage(groupJid, {
            text: quote(`⚠️ Hay un error: ${error.message}`)
        });
    }
}
...
module.exports = (bot) => {
    ...
    bot.ev.once(Events.ClientReady, async (m) => {
        ...
        if (botRestart?.jid && botRestart?.timestamp) {
            ...
            await bot.core.sendMessage(botRestart.jid, {
                text: quote(`✅ ¡Reiniciado exitosamente! Toma tiempo ${timeago}.`),
                edit: botRestart.key
            });
        ...
        config.bot = {
            ...
            readyAt: bot.readyAt,
            groupLink: await bot.core.groupInviteCode(config.bot.groupJid).then(code => `https://chat.whatsapp.com/${code}`).catch(() => "https://chat.whatsapp.com/EfnOxpPAjsr5FCm5M3m8Dd")
        };
    });
    ...
    bot.ev.on(Events.MessagesUpsert, async (m, ctx) => {
        ...
        isGroup ? consolefy.info(`Mensaje entrante del grupo: ${groupId}, by: ${senderId}`) : consolefy.info(`Mensaje entrante de: ${senderId}`); // Log pesan masuk
        ...
        if (isGroup || isPrivate) {
            ...
            if (isCmd?.didyoumean) await ctx.reply(quote(`❎ Parece que cometiste un error tipográfico. ${monospace(isCmd?.prefix + isCmd?.didyoumean)}.`)); // Did you mean?
            ...
            if (userAFKJids.length > 0) {
                ...
                for (const userAFKJid of userAFKJids) {
                    ...
                    if (userAFK?.reason && userAFK?.timestamp) {
                        ...
                        await ctx.reply(quote(`📴 Él está AFK ${userAFK.reason ? `razon: "${userAFK.reason}"` : "ninguna"} por ${timeago}.`));
                    }
                }
            }
            ...
            if (userAFK?.reason && userAFK?.timestamp) {
                ...
                if (timeElapsed > 3000) {
                    ...
                    await ctx.reply(quote(`📴 Ya no esta AFK ${userAFK.reason ? `razon "${userAFK.reason}"` : "ninguna"} durante ${timeago}.`));
                }
            }
        }
        ...
        if (isGroup) {
            ...
            if (groupDb?.option?.antilink && await tools.general.isUrl(m.content) && !await ctx.group().isSenderAdmin()) {
                ...
                await ctx.reply(quote("⛔ ¡No envíes enlaces!"));
            ...
            if (groupDb?.option?.antinsfw) {
                ...
                if (checkMedia && !await ctx.group().isSenderAdmin()) {
                    ...
                    if (result === "nsfw") {
                        await ctx.deleteMessage(m.key);
                        await ctx.reply("⛔ ¡No envíes NSFW!");
                        if (!config.system.restrict && groupDb?.option?.autokick) await ctx.group().kick([ctx.sender.jid]);
                    }
                }
            }
            ...
            if (groupDb?.option?.antispam) {
                ...
                if (newCount > 5 && !await ctx.group().isSenderAdmin()) {
                    await ctx.deleteMessage(m.key);
                    await ctx.reply(quote("⛔ ¡No envíes spam!"));
                    if (!config.system.restrict && groupDb?.option?.autokick) await ctx.group().kick([ctx.sender.jid]);
            ...
            for (const type of ["audio", "document", "gif", "image", "sticker", "video"]) {
                if (groupDb?.option?.[`anti${type}`]) {
                    ...
                    if (checkMedia && !await ctx.group().isSenderAdmin()) {
                        await ctx.deleteMessage(m.key);
                        await ctx.reply(`⛔ ¡No envíes ${type}!`);
                        if (!config.system.restrict && groupDb?.option?.autokick) await ctx.group().kick([ctx.sender.jid]);
                    }
                }
            }
            ...
            if (groupDb?.option?.antitoxic) {
                ...
                if (m.content && toxicRegex.test(m.content) && !await ctx.group().isSenderAdmin()) {
                    await ctx.deleteMessage(m.key);
                    await ctx.reply(quote("⛔ ¡No seas tóxico!"));
                    if (!config.system.restrict && groupDb?.option?.autokick) await ctx.group().kick([ctx.sender.jid]);
                }
            }
        }
                ...
                for (const [conversationId, {
                    ...
                    if (senderId === from || senderId === to) {
                        ...
                        if (m.content?.match(/\b(delete|stop)\b/i)) {
                            const replyText = quote("✅ ¡El mensaje de confección ha sido eliminado!");
    ...
    bot.ev.on(Events.Call, async (calls) => {
        ...
        for (let call of calls) {
            ...
            let rejectionMessage = await bot.core.sendMessage(call.from, {
                text: `En este momento no podemos aceptar llamadas. ${call.isVideo ? "video" : "voz"}.\n` +
                    "Si necesita ayuda, ¡póngase en contacto con el propietario!"
            });
