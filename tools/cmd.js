...
function generateInstruction(actions, mediaTypes) {
    if (!actions || !actions.length) return "'¡Se deben especificar las 'acciones requeridas'!";
    ...
    if (typeof mediaTypes === "string") {
        ...
        return "¡'mediaTypes' debe ser una cadena o una matriz de cadenas!";
    }
    ...
    if (translatedMediaTypeList.length > 1) {
        ...
        mediaTypesList = translatedMediaTypeList.slice(0, -1).join(", ") + `, o ${lastMediaType}`;
    } else {
    ...
    const actionList = instructions.join(actions.length > 1 ? " o " : "");

    return `📌 ${actionList} ${mediaTypesList}!`;
}

function generateCommandExample(used, args) {
    if (!used) return "¡Se debe indicar 'usado'!";

    if (!args) return "¡Se debe proporcionar 'un texto'!";

    const commandMessage = `Ejemplo: ${monospace(`${used.prefix + used.command} ${args}`)}`;
    return commandMessage;
}

function generatesFlagInformation(flags) {
    if (typeof flags !== "object" || !flags) return "'Bandera' debe ser un objeto!";

    const flagInfo = "Bandera:\n" +
...
function generateNotes(notes) {
    if (!Array.isArray(notes)) return "¡'Nota' debe ser una cadena!";

    const notesInfo = "Notas:\n" +
...
async function handleError(ctx, error, useAxios) {
    ...
    return await ctx.reply(quote(`⚠️ Hay un error: ${error.message}`));
}
