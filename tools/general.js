...
function convertMsToDuration(ms) {
    ...
    if (weeks > 0) durationString += weeks + " semana(s) ";
    if (days > 0) durationString += days + " día(s) ";
    if (hours > 0) durationString += hours + " hora(s) ";
    if (minutes > 0) durationString += minutes + " minuto(s) ";
    if (seconds > 0) durationString += seconds + " segundo(s)";
...
async function upload(buffer, type = "any", host = config.system.uploaderHost) {
    ...
    if (!realHost) return `El Host '${host}' no admite el tipo '${type}'`;

    try {
        const url = await uploader[realHost](buffer);
        return url || `No se pudo cargar a '${realHost}'`;
    } catch (error) {
