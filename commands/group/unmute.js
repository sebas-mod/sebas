...
module.exports = {
    ...
    code: async (ctx) => {
        ...
        if (ctx.args[0] === "bot") {
            ...
            return await ctx.reply(quote("✅ ¡Esta bot fue silenciada!"));
        }

        if (!accountJid) return await ctx.reply({
            text: `${quote(tools.cmd.generateInstruction(["send"], ["text"]))}\n` +
                `${quote(tools.cmd.generateCommandExample(ctx.used, `@${senderId}`))}\n` +
                quote(tools.cmd.generateNotes(["Responda o cite el mensaje para que el remitente sea la cuenta de destino.", `Escribe ${monospace(`${ctx.used.prefix + ctx.used.command} bot`)} para reactivar la bot.`])),
            mentions: [senderJid]
        });
