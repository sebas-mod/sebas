    ...
    bot.use(async (ctx, next) => {
        ...
        if (newUserXp >= xpToLevelUp) {
            ...
            if (userDb?.autolevelup) {
                ...
                const text = `${quote(`¡Felicitaciones! Has subido de nivel ${newUserLevel}!`)}\n` +
                    `${config.msg.readmore}\n` +
                    quote(tools.cmd.generateNotes([`¿Hay algo mal? Escribe ${monospace(`${ctx.used.prefix}setprofile autolevelup`)} para deshabilitar los mensajes de nivelación automática.`]));
